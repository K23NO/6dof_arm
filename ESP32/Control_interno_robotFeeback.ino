...
// Parámetros de control PID externo basado en retroalimentación
const float CONTROL_KP = 1.2f;
const float CONTROL_KI = 0.02f;
const float CONTROL_KD = 0.05f;
const float CONTROL_DT = 0.1f;  // 100 ms
const float CONTROL_INTEGRAL_LIMIT = 150.0f;  // anti-windup
const float COMMAND_MAX_STEP = 12.0f;  // límite de cambio por iteración (deg)
...
float target_angles[N_SERVOS] = {0};
float command_angles[N_SERVOS] = {0};
float control_integral[N_SERVOS] = {0};
float previous_error[N_SERVOS] = {0};
...
void subscription_callback(const void * msgin)
{
  ...
  for (size_t i = 0; i < n; i++) {
    ...
    float previous_target = target_angles[i];
    target_angles[i] = (float)angle;

    // Reiniciar integral si hubo un cambio grande en la referencia
    if (fabsf(target_angles[i] - previous_target) > 2.0f) {
      control_integral[i] = 0.0f;
...
void timer_callback(rcl_timer_t * timer, int64_t last_call_time)
{
  ...
  if (timer != NULL) {
    ...
    for (int i = 0; i < N_SERVOS; i++) {
      ...
      float error = target_angles[i] - (float)filtered_angle;
      control_integral[i] += error * CONTROL_DT;
      control_integral[i] = constrain(control_integral[i], -CONTROL_INTEGRAL_LIMIT, CONTROL_INTEGRAL_LIMIT);
      float derivative = (error - previous_error[i]) / CONTROL_DT;
      float control_signal = (CONTROL_KP * error) + (CONTROL_KI * control_integral[i]) + (CONTROL_KD * derivative);

      float desired_angle = target_angles[i] + control_signal;
      desired_angle = constrain(desired_angle, (float)SERVO_MIN_ANGLES[i], (float)SERVO_MAX_ANGLES[i]);

      float delta = desired_angle - command_angles[i];
      if (delta > COMMAND_MAX_STEP) delta = COMMAND_MAX_STEP;
      if (delta < -COMMAND_MAX_STEP) delta = -COMMAND_MAX_STEP;

      command_angles[i] += delta;
      command_angles[i] = constrain(command_angles[i], (float)SERVO_MIN_ANGLES[i], (float)SERVO_MAX_ANGLES[i]);

      servos[i].write((int)roundf(command_angles[i]));
      previous_error[i] = error;

      // Convertir a radianes respecto al home físico con el ángulo medido
...
void setup() {
  ...
  for (int i = 0; i < N_SERVOS; i++) {
    ...
    target_angles[i] = (float)SERVO_HOME_ANGLES[i];
    command_angles[i] = (float)SERVO_HOME_ANGLES[i];
    control_integral[i] = 0.0f;
    previous_error[i] = 0.0f;
  }
  ...
  for (int i = 0; i < N_SERVOS; i++) {
    ...
    servos[i].write((int)SERVO_HOME_ANGLES[i]);
